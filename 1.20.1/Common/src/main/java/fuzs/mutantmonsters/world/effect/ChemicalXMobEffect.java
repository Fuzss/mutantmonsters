...
public class ChemicalXMobEffect extends InstantenousMobEffect {
    public static final Predicate<LivingEntity> IS_APPLICABLE = (target) -> {
        ...
        return !CommonAbstractions.INSTANCE.isBossMob(entityType) && !MutantMonsters.CONFIG.get(ServerConfig.class).mutantXConversions.containsValue(entityType) && entityType != ModRegistry.CREEPER_MINION_ENTITY_TYPE.get() && entityType != ModRegistry.ENDERSOUL_CLONE_ENTITY_TYPE.get();
    };
    ...
    public static EntityType<?> getMutantOf(LivingEntity target) {
        ...
        if (!MutantMonsters.CONFIG.get(ServerConfig.class).mutantXConversions.containsKey(targetType) || target.isBaby()) {
            return null;
