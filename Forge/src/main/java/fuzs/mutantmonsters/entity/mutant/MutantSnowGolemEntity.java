...
public class MutantSnowGolemEntity extends AbstractGolem implements RangedAttackMob, IForgeShearable {
    ...
    public MutantSnowGolemEntity(EntityType<? extends MutantSnowGolemEntity> type, Level worldIn) {
        super(type, worldIn);
    ...
    public void tick() {
        ...
        if (!this.level.isClientSide && this.onGround && !this.level.dimensionType().ultraWarm() && ForgeEventFactory.getMobGriefingEvent(this.level, this)) {
    ...
    public static boolean canHarm(Entity attacker, Entity target) {
        if (!(attacker instanceof MutantSnowGolemEntity)) {
            ...
            return !((CreeperMinionEntity)target).isTame();
        } else {
            return target instanceof Enemy || target instanceof Mob && ((Mob)target).getTarget() == attacker || ((MutantSnowGolemEntity)attacker).getTarget() == target;
        }
    }
    ...
    static {
        OWNER_UNIQUE_ID = SynchedEntityData.defineId(MutantSnowGolemEntity.class, EntityDataSerializers.OPTIONAL_UUID);
        STATUS = SynchedEntityData.defineId(MutantSnowGolemEntity.class, EntityDataSerializers.BYTE);
    }
    ...
    class ThrowIceGoal extends Goal {
        ...
        public boolean canUse() {
            this.attackTarget = MutantSnowGolemEntity.this.getTarget();
        ...
        @Override
        public void tick() {
            MutantSnowGolemEntity.this.getNavigation().stop();
            MutantSnowGolemEntity.this.yBodyRot = MutantSnowGolemEntity.this.getYRot();
            if (MutantSnowGolemEntity.this.throwingTick == 7) {
                ...
                MutantSnowGolemEntity.this.level.addFreshEntity(block);
            }
    ...
    static class SwimJumpGoal extends Goal {
        ...
        public void tick() {
            --this.jumpTick;
            if (!this.waterReplaced && !this.golem.isInWater() && this.jumpTick < 17 && ForgeEventFactory.getMobGriefingEvent(this.golem.level, this.golem)) {
