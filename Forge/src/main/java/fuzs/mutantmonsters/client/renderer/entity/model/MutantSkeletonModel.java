...
public class MutantSkeletonModel extends EntityModel<MutantSkeletonEntity> {
    ...
    public MutantSkeletonModel(ModelPart modelPart, ModelPart crossbowModelPart) {
        this.parts = Stream.of(modelPart, crossbowModelPart).flatMap(ModelPart::getAllParts).filter(Predicate.not(ModelPart::isEmpty)).collect(ImmutableList.toImmutableList());
    ...
    private void animate(MutantSkeletonEntity skele, float limbSwing, float limbSwingAmount, float ageInTicks, float netHeadYaw, float headPitch) {
