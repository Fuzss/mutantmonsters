...
import fuzs.mutantmonsters.client.renderer.entity.model.MutantSkeletonModel;
...
public class MutantSkeletonRenderer extends MobRenderer<MutantSkeletonEntity, MutantSkeletonModel> {
    ...
    @Override
    protected float getFlipDegrees(MutantSkeletonEntity entityLivingBaseIn) {
        return 0.0F;
    }

    @Override
    public ResourceLocation getTextureLocation(MutantSkeletonEntity entity) {
        return TEXTURE;
    }

    static class CrossbowLayer extends RenderLayer<MutantSkeletonEntity, MutantSkeletonModel> {
        private final MutantArrowModel model;

        public CrossbowLayer(RenderLayerParent<MutantSkeletonEntity, MutantSkeletonModel> entityRendererIn, EntityModelSet entityModelSet) {
            super(entityRendererIn);
            this.model = new MutantArrowModel(entityModelSet.bakeLayer(ClientModRegistry.MUTANT_ARROW));
        }

        @Override
        public void render(PoseStack matrixStackIn, MultiBufferSource bufferIn, int packedLightIn, MutantSkeletonEntity entity, float limbSwing, float limbSwingAmount, float partialTicks, float ageInTicks, float netHeadYaw, float headPitch) {
